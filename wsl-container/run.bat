@echo off
rem =============================================================================
rem run.bat - Launcher for wsl-container\run.ps1
rem
rem All logic lives in run.ps1. This wrapper only exists so the script can be
rem started from Command Prompt or by double-clicking, without relaxing the
rem PowerShell execution policy machine-wide.
rem
rem Administrator elevation is handled by run.ps1 itself (UAC prompt), so this
...
rem Japanese messages therefore live in run.ps1, saved as UTF-8 with BOM.
rem
rem Usage:
rem   wsl-container\run.bat ^<command^> [busid]
rem
...
rem to "setup"; running "run.bat" from a prompt still shows the usage text.
set "PS_ARGS=%*"
if defined LAUNCHED_BY_EXPLORER set "PS_ARGS=setup -FromDoubleClick"

powershell.exe -NoProfile -ExecutionPolicy Bypass -File "%~dp0run.ps1" !PS_ARGS!
set "RC=!ERRORLEVEL!"
